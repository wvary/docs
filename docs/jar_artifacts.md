...
In addition to uploading JAR files and creating new schemas manually, you can write JAR files directly to the local file system designated as artifacts directories and the Gateway processes these JAR files and writes them to HDFS.  You must configure the `dt.gateway.maven.local.repo` property in the `dt-site.xml` file for this synchronization process to work.
